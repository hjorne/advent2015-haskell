...
day9 :: IO ()
day9 = parseFromFile (some parseLine) infile >>= \dists ->
    print (part1 $ fromJust dists) >> print (part2 $ fromJust dists)
...
parseLine =
    mkEdge <$> parseID <*> symbol "to" <*> parseID <*> symbolic '=' <*> natural
  where
    mkEdge id1 _ id2 _ dist = ((id1, id2), dist)
    parseID = token $ some letter

doubleE :: [(Edge, Integer)] -> [(Edge, Integer)]
doubleE = foldr go []
  where
    go x acc = x : swap x : acc
...
deleteL :: Int -> [a] -> [a]
deleteL i xs = left ++ tail right where (left, right) = splitAt i xs
